...
function registerExtension() {
  ...
  // SAFETY: registerVitePlus registers exactly one `before_agent_start` handler via `on`;
  // we capture it and fail the test if it never arrives.
  registerVitePlus({
...
describe("Vite+ guidance", () => {
  ...
  it.each([
    ...
    "`vp <name>` runs a built-in command",
    "`vp run <name>` runs a `package.json` script or a `vite.config.ts` task",
    "Scripts cannot overwrite built-ins",
    "`vp dev` and `vp run dev` may do different things",
    "Check `package.json` and `vite.config.ts` first",
    "## Review Checklist",
