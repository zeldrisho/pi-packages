...
`maxCharacters` bounds returned Markdown (default 6,000); it does not change the raw download limit. `offset` is a character offset into extracted content. A URL fragment such as `https://github.com/shadcn-ui/lint#get-started` starts at the matching heading or anchor (the complete page is still fetched and cached); an explicit `offset` takes precedence. For long pages, pass `query` to get a deterministic, source-ordered focused view selected locally after the complete page is fetched and cached. `details.focus` reports matched, total, and omitted sections plus character counts. Omit `query` to read the complete document. Use a returned `nextOffset` for continuation.
